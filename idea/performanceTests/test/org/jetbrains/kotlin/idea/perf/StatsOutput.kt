...
internal fun Metric.writeJson() {
    val statsFile = statsFile((name ?: childrenName), "json")
    val buildId = properties?.get("build_id")
    statsFile.bufferedWriter().use { output ->
        buildId?.let {
            output.appendLine("{\"index\":{\"_id\":\"$buildId\"}}")
        }
...
private fun Metric.toJson(prefix: String): String =
    buildString {
        ...
        if (name != null) {
            ...
            s = "$prefix $childrenName${if (prefix.isEmpty()) ":" else ""}".trim()
        }
